...
- `SmoothScroll` — owns the single Lenis instance, exposed as `window.__nxrLenis`; **any programmatic scrolling must go through it** (see `Servicios.tsx`). Also calls `ScrollTrigger.config({ ignoreMobileResize: true })` and uses `syncTouch: true` so touch scroll advances in the same rAF the scene reads. **That flag is load-bearing and was tried off in V18.67, reverted in V18.70**: off, mobile browsers collapse their toolbar again (they only do that for finger-driven scrolls, never programmatic ones), but the compositor then scrolls content ahead of the main thread and every DOM-anchored glass mesh visibly trails its own card's text. There is no middle ground — the compositor's applied offset is unreadable from JS — and the third way (painting the glass in CSS on the card itself, V18.68-69) didn't match the material. Glass wins; the toolbar stays put.
